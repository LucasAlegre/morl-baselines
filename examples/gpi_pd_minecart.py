...
from rl.utils.utils import (extrema_weights, random_weights, seed_everything,
                            unique_tol)
...
def main(algo: str, gpi_pd: bool, g: int, timesteps_per_iter: int = 10000, seed: int = 0):
    ...
    test_tasks = random_weights(dim=3, seed=42, n=100-3) + extrema_weights(3)
    ref_point = np.array([0.0, 0.0, -11.0])
    ccs = eval_env.convex_coverage_set(frame_skip=4, discount=0.98, incremental_frame_skip=True, symmetric=True)
    max_iter = 15
    for iter in range(1, max_iter + 1):
        if algo == 'ols' or algo == 'gpi-ls':
            if algo == 'gpi-ls':
                agent.set_weight_support(unique_tol(linear_support.get_weight_support()))
        ...
        if algo != 'envelope':
            gpi_returns_test_tasks = [policy_evaluation_mo(agent, eval_env, w, rep=5, return_scalarized_value=False) for w in test_tasks]
            ...
            max_gap_gpi = max([np.dot(best_vector(ccs, w),w) - np.dot(best_vector(gpi_returns_test_tasks, w),w) for w in test_tasks])
            wb.log({"eval/Max Gap GPI": max_gap_gpi, "iteration": iter})

        agent.use_gpi = False
        no_gpi_returns_test_tasks = [policy_evaluation_mo(agent, eval_env, w, rep=5, return_scalarized_value=False) for w in test_tasks]
        mean_no_gpi_returns_test_tasks = np.mean([np.dot(psi, w) for (psi, w) in zip(no_gpi_returns_test_tasks, test_tasks)], axis=0)
        agent.use_gpi = algo != 'envelope'
        wb.log({"eval/Mean Utility No GPI": mean_no_gpi_returns_test_tasks, "iteration": iter})
        max_gap_no_gpi = max([np.dot(best_vector(ccs, w),w) - np.dot(best_vector(no_gpi_returns_test_tasks, w),w) for w in test_tasks])
